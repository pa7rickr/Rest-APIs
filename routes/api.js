...
loghandler = {
	...
	error: {
		status: false,
		creator: creator,
		message: 'Error, please chat owner!'
	}
}
...
const listkey = ["simple", "qwerty"];

router.post("/addedapikeyqwerty", async (req, res, next) => {
	const key = req.query.key;
	if (listkey.includes(key)) {
		res.json({
			message: 'Apikey tersebut telah terdaftar'
		});
	} else {
		listkey.push(key);
		res.json({
...
router.delete("/deletedapikeyqwerty", async (req, res, next) => {
	const key = req.query.delete;
	if (listkey.includes(key)) {
		res.json({
			message: 'Apikey tersebut tidak terdaftar'
		})
	} else {
		listkey.splice(key, 1)
		res.json({
			message: 'Succes menghapus Apikey!'
		});
	}
});
...
router.get('/kuis/family100', async (req, res, next) => {
	...
	if (!Apikey) return res.json(loghandler.notparam)
	if (listkey.includes(Apikey)) {
		const femm = JSON.parse(fs.readFileSync(__path + '/data/family100.json'));
		var result = femm[Math.floor(Math.random() * femm.length)];
		res.json({
				status: true,
				code: 200,
				creator: creator,
...
router.get('/kuis/tebakbendera', async (req, res, next) => {
	...
	if (!Apikey) return res.json(loghandler.notparam)
	if (listkey.includes(Apikey)) {
		const flags = JSON.parse(fs.readFileSync(__path + '/data/tebakbendera.json'));
		var result = flags[Math.floor(Math.random() * flags.length)];
		res.json({
				status: true,
				code: 200,
				creator: creator,
...
router.get('/kuis/tod', async (req, res, next) => {
	Apikey = req.query.apikey;
	if (!Apikey) return res.json(loghandler.notparam)
	if (listkey.includes(Apikey)) {
		const terut = JSON.parse(fs.readFileSync(__path + '/data/truth.json'));
		const dere = JSON.parse(fs.readFileSync(__path + '/data/dare.json'));
		var truth = terut[Math.floor(Math.random() * terut.length)];
		var dare = dere[Math.floor(Math.random() * dere.length)];
		const wikwog = {
				status: true,
				code: 200,
				creator: creator,
...
router.get('/random/wallpaper/cyberspace', async (req, res, next) => {
	var Apikey = req.query.apikey
...
router.get('/random/wallpaper/teknologi', async (req, res, next) => {
	const Apikey = req.query.apikey;
	if (!Apikey) return res.json(loghandler.notparam)
	if (listkey.includes(Apikey)) {
...
router.get('/random/wallpaper/muslim', async (req, res, next) => {
	var Apikey = req.query.apikey
...
router.get('/random/wallpaper/programming', async (req, res, next) => {
	var Apikey = req.query.apikey
...
router.get('/random/wallpaper/pegunungan', async (req, res, next) => {
	var Apikey = req.query.apikey
...
router.get('/kuis/caklontong', async (req, res, next) => {
	...
	if (!Apikey) return res.json(loghandler.notparam)
	if (listkey.includes(Apikey)) {
		const caklent = JSON.parse(fs.readFileSync(__path + '/data/caklontong.json'));
		var result = caklent[Math.floor(Math.random() * caklent.length)];
		res.json({
				status: true,
				code: 200,
				creator: creator,
...
router.get('/maker/ttp', async (req, res, next) => {
	...
	Apikey = req.query.apikey;
	if (!req.query.text) return res.json({
		status: 404,
...
router.use(function(req, res) {
	...
	res.status(404)
		...
		.sendFile(__path + '/examples/404.html');
});
